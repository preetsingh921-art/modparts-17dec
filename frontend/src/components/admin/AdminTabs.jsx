...
const AdminTabs = () => {
  ...
  // Define which tabs are superadmin-only
  const tabs = [
    {
      to: '/admin',
      label: 'Dashboard',
  ...
  const LockIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-[#B8860B]" viewBox="0 0 20 20" fill="currentColor">
          ...
          >
            {tabs.map((tab) => (
              <option key={tab.to} value={tab.to}>
                {tab.label}{tab.superadminOnly && !isUserSuperAdmin ? ' 🔒' : ''}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="hidden md:block overflow-x-auto scrollbar-hide">
          <nav className="flex space-x-1 min-w-max" aria-label="Tabs">
            {tabs.map((tab) => {
              ...
              const isLocked = tab.superadminOnly && !isUserSuperAdmin;
              ...
              return (
                <Link
                  ...
                  className={`flex items-center space-x-1.5 py-3 px-3 lg:px-4 border-b-3 text-sm transition-all uppercase tracking-wide whitespace-nowrap ${isActive
                    ...
                    : isLocked
                      ? 'border-transparent text-[#555] hover:text-[#777] hover:bg-[#222]/50'
                      : 'border-transparent text-[#A8A090] hover:text-[#F5F0E1] hover:bg-[#333]/50'
                    }`}
                  style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 500 }}
                  title={isLocked ? `${tab.label} — Super Admin only` : tab.label}
                >
                  <span className={isLocked ? 'opacity-40' : ''}>{tab.icon}</span>
                  <span className={`hidden lg:inline ${isLocked ? 'opacity-40' : ''}`}>{tab.label}</span>
                  <span className={`lg:hidden ${isLocked ? 'opacity-40' : ''}`}>{tab.label.substring(0, 4)}</span>
                  {isLocked && <LockIcon />}
                </Link>
              );
            })}
