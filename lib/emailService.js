...
// Create Gmail transporter (free and reliable)
const createTransporter = () => {
  const gmailUser = process.env.GMAIL_USER || process.env.SMTP_USER || process.env.EMAIL_USER;
  const gmailPass = (process.env.GMAIL_APP_PASSWORD || process.env.SMTP_PASS || process.env.EMAIL_PASSWORD || '').replace(/\s/g, ''); // Remove spaces from app password
...
const sendOrderConfirmationEmail = async (orderData) => {
  try {
    ...
    const customerName = `${first_name || ''} ${last_name || ''}`.trim() || 'Customer';
    const formattedTotal = Number(total_amount || 0).toFixed(2);

    const itemsHtml = items.map(item => `
      <tr>
        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${item.product_name || item.name || 'Part #' + (item.product_id || '')}</td>
    ...
    const mailOptions = {
      from: `"${process.env.EMAIL_FROM_NAME || 'Sardaarji Auto Parts'}" <${process.env.GMAIL_USER || process.env.SMTP_USER || process.env.EMAIL_USER}>`,
      to: email,
      subject: `Order Confirmation #${orderId} - Sardaarji Auto Parts`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <meta charset="utf-8">
          <style>
            body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; background-color: #1a1a1a; }
            .container { max-width: 600px; margin: 20px auto; background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.3); }
            .header { background: #8B2332; color: #F5F0E1; padding: 25px; text-align: center; }
            .content { padding: 30px; }
            .table { width: 100%; border-collapse: collapse; margin: 20px 0; }
            .table th { background: #f4f4f4; padding: 10px; text-align: left; font-size: 13px; text-transform: uppercase; color: #555; }
            .total-box { background: #f9fafb; padding: 15px; border-radius: 6px; margin: 20px 0; text-align: right; }
            .footer { padding: 20px; text-align: center; color: #777; font-size: 12px; background: #f4f4f4; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin:0; font-size: 24px; letter-spacing: 1px;">SARDAARJI AUTO PARTS</h1>
              <p style="margin:5px 0 0 0; font-size: 14px; color: #D4A84B;">Order #${orderId} Confirmed</p>
            </div>
            <div class="content">
              ...
              <div class="total-box">
                <p style="margin: 0; font-size: 18px; font-weight: bold; color: #8B2332;">Total: $${formattedTotal}</p>
                <p style="margin: 5px 0 0 0; font-size: 13px; color: #666;">Payment Method: ${payment_method || 'Standard'}</p>
              </div>

              ${shipping_address ? `
              <div style="margin-top: 20px; padding: 15px; background: #fff8f0; border-left: 4px solid #B8860B; border-radius: 4px;">
                <strong style="color: #92400e;">Shipping Address:</strong>
                <p style="margin: 5px 0 0 0; font-size: 14px; color: #444;">${shipping_address}</p>
              </div>
              ` : ''}
            ...
            <div class="footer">
              <p>© ${new Date().getFullYear()} Sardaarji Auto Parts — Vintage Motorcycle Specialists</p>
            </div>
          </div>
        </body>
        </html>
    ...
    const result = await transporter.sendMail(mailOptions);
    console.log(`✅ Order confirmation email sent to ${email} for order #${orderId}:`, result.messageId);
    return { success: true, messageId: result.messageId };
  } catch (error) {
